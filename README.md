...
| 30 | [Seo et al. 2024](https://arxiv.org/abs/2502.05221) | Diffusion Models | TSP | Integrates Blackout Diffusion into DIFUSCO; introduces continuous-time modeling, linear observation scheduling, and fine-grained time slicing. | Validates the approach; performance comparable to baseline; first application of Blackout Diffusion to CO. | [Available](https://github.com/Giventicket/BlackoutDIFUSCO) |
| 31 | [Graikos et al. 2022](https://proceedings.neurips.cc/paper_files/paper/2022/hash/5e6cec2a9520708381fe520246018e8b-Abstract-Conference.html) | Diffusion Models | TSP, image segmentation | Uses pre-trained DDPM as plug-and-play prior; combines with differentiable constraints; encodes adjacency matrices as images. | Achieves competitive optimality gaps on TSP-50/100; demonstrates reasoning in image space for CO. | [Available](https://github.com/AlexGraikos/diffusion_priors) |
| 1 | [Li et al. 2025]() | Diffusion Models | TSP, MIS | Fast T2T: Optimization consistency training for direct noise-to-solution mapping, with gradient search in testing. | Outperforms SOTA diffusion solvers with 1-step generation, achieving 35–82× speedup; beats LKH in early stages. | [Available](https://github.com/Thinklab-SJTU/Fast-T2T) |
| 2 | [Zeng et al. 2024]() | Diffusion Models | Integer Programming  | Guided diffusion with contrastive pre-training (CISP), constraint- & objective-guided sampling (DDIM/DDPM). | Generates complete feasible solutions (>89.7% feasible ratio), outperforms Neural Diving and PS; 3.7–33.7% gap improvement. | [Available](https://github.com/agent-lab/diffusion-integer-programming) |
| 1 | [Sanokowski et al. 2025]() | Diffusion Models | Ising model, MIS, MaxCut, MDS, MaxClique | SDDS: Scalable Discrete Diffusion Samplers with two training objectives (reverse KL with RL & forward KL with MC); enables unbiased sampling via adapted SN-NIS & NMCMC. | SOTA on 6/7 UCO benchmarks; outperforms autoregressive models in unbiased Ising sampling. | [Available](https://github.com/ml-jku/DIffUCO) |
| 2 | [Li et al. 2024] () | Diffusion Models | TSP, MIS | T2T: Training diffusion model for solution distribution learning; testing with gradient-guided denoising for local search. | 49.15% avg gain on TSP, 17.27% on MIS vs. SOTA; better generalization & label efficiency. | [Available](https://github.com/Thinklab-SJTU/T2TC0) |
| 7 | [Wang et al. 2023](https://arxiv.org/abs/2501.13767) | GANs | General COPs | GIRL framework: Integration of inverse RL with GANs, self-attention-based policy network generates 2-opt heuristic rules | Addresses sparse-reward issue, mitigates training instability | [Available](https://github.com/DEITSP/DEITSP) |
| 8 | [Zhang et al. 2025](https://arxiv.org/abs/2503.01931) | GANs | Routing Problems | Adversarial Generative Flow Network: Replaces Transformer with generative flow network, discriminator contrasts original and optimized solutions | Breaks scalability bottleneck, produces diverse routing solutions | [Available](https://github.com/ZHANG-NI/AGFN) |
| 9 | [Guo et al. 2020](https://doi.org/10.1007/978-3-030-53956-6_38) | GANs | Multi-objective Optimization | Multi-objective combinatorial GAN: Uses non-dominated solutions as real data, adaptive parameter tuning, decimal-encoding decoding | 35% speedup over NSGA-II on group-aware participant selection | |
| 10 | [Chen et al. 2019](https://doi.org/10.1145/3331453.3361639) | GANs | General COPs | DCG-EA/I: Negative-sample exploitation mechanism, reconstructs low-quality solutions into pseudo-positive samples, integrates 2-opt local search | Addresses mode collapse issue, enhances solution feasibility | |
| 11 | [Yu et al. 2022](https://ieeexplore.ieee.org/abstract/document/10076625) | GANs | Discrete Optimization | PGAN-CEA: Combines policy gradients with Monte Carlo simulation | Improves GANs' ability to generate high-quality discrete solutions | |
| 12 | [Zhang et al. 2024](https://doi.org/10.1038/s41598-024-56699-7) | GANs | 3D Bin Packing | Hybrid GAN-GA model: Generates spatial item layouts | Improves packing utilization by 4.7% | |
| 13 | [Bentley et al. 2023](https://doi.org/10.1145/3583131.3590459) | VAEs | Robotic Scheduling | COIL: Integration of VAEs with optimizers, encodes historical schedules into latent space, GA optimization in latent space | Satisfies robot parallelism constraints through latent space encoding | |
| 14 | [Onsu et al. 2025](https://ieeexplore.ieee.org/abstract/document/11161524) | VAEs | Service Function Chain Deployment | Generative VAE-assisted DRL for 5G networks, compresses network states into latent representations, end-to-end latency penalty term | Optimizes SFC deployment with sub-10 ms delay requirement | |
| 15 | [Chen et al. 2023](https://kns.cnki.net/kcms2/article/abstract?v=hmNf6PfbaBvBLBKas-nmtk3HqbB72omJrpgO0drevsyv2iRKEvQ-m3y6vPb1sVQmK8eIc66plA6IeUqU1y0GZURTFuA8V8HuUtqNqTkJaac_NYES-ruobSIEHelXPtWCQvsd3Q85t_uT-B3EnO8IG4unrvsh3mBgWW03FzPgh8wexL91wtCjEA==&uniplatform=NZKPT&language=CHS) | VAEs | Active Distribution Networks | VAE-enhanced TD3 algorithm, encodes multi-site energy-storage states into latent variables capturing spatio-temporal coupling | Minimizes overall system operating costs | |
| 16 | [Bhattacharjee et al. 2019](https://ieeexplore.ieee.org/abstract/document/8790077) | VAEs | Complex Benchmark Problems (Trap-k) | VAE-EDA-Q: Embeds VAE into estimation-of-distribution algorithm, adaptive variance scaling for sampling diversity | 5x speedup in convergence over BOA algorithm on Trap-13 | |
| 17 | [Wang et al. 2024](https://ieeexplore.ieee.org/abstract/document/10756255) | VAEs | Task Offloading and Resource Allocation | CARMARL: Conditional VAE for hybrid discrete-continuous decisions, reward-constrained policy optimization | 25.2% increase in completion rate of intelligent connected vehicle tasks | |
| 18 | [Yao et al. 2019](https://ieeexplore.ieee.org/abstract/document/9008496) | VAEs | Content Pre-caching | CVAE for user mobility trajectory prediction, optimizes small-cell content pre-caching strategies | Reduces handover latency | |
| 19 | [Nouri et al. 2024](https://arxiv.org/abs/2401.08861) | VAEs | O-RAN Slicing | Semi-supervised resource allocator: Combines VAEs with contrastive learning | 14% improvement in physical resource block allocation accuracy | |
| 20 | [Li et al. 2024](https://ieeexplore.ieee.org/abstract/document/10643661) | VAEs | Industrial Sensor Resource Allocation | DT-VAE evaluation mechanism based on digital twins, virtual-physical consistency analysis | Improves cooperative sensing accuracy | |
| 21 | [Shanmugham et al. 2024](https://doi.org/10.1016/j.eswa.2023.122097) | VAEs | MANETs Routing | Self-attention-based CVAE-GAN routing paradigm, strengthens node state representations | 22% reduction in energy consumption for multipath routing | |
| 22 | [Yao et al. 2019](https://doi.org/10.1117/12.2529608) | Discriminative Models | MAX-CUT | Investigation of unsupervised GNNs on random MAX-CUT, approximates SDP benchmark without label supervision | Demonstrates GNNs can approximate SDP benchmark | |
| 23 | [Karalias et al. 2020](https://proceedings.neurips.cc/paper/2020/hash/49f85a9ed090b20c8bed85a5923c669f-Abstract.html) | Discriminative Models | Cliques, Vertex Covers, Independent Sets | Mathematically grounded unsupervised framework, parameterizes distribution over sets, derandomization strategy | Provides theoretical guarantees for solution quality and feasibility | |
| 24 | [Schuetz et al. 2022](https://www.nature.com/articles/s42256-022-00468-6) | Discriminative Models | QUBO Problems | Physics-inspired GNN: Hamiltonian minimization, reformulates COPs as QUBO tasks, continuous relaxation for binary variables | Lays theoretical foundation for directly solving COPs with GNNs |[Available](https://github.com/amazon-science/co-with-gnns-example) |
| 25 | [Toenshoff et al. 2021](https://www.frontiersin.org/journals/artificial-intelligence/articles/10.3389/frai.2020.580607/full) | Discriminative Models | Binary Max Constraint Satisfaction | Unsupervised recurrent GNN, constraint graph as substrate, single-LSTM unit with linear layers | Directly maximizes number of satisfied constraints | [Available](https://github.com/toenshoff/RUN-CSP) |
| 26 | [Boisvert et al. 2024](https://link.springer.com/chapter/10.1007/978-3-031-60597-0_7) | Discriminative Models | SAT, Graph Coloring, TSP, Knapsack | AST-based encoding framework, generic injective graph representation function, unifies expression of diverse problems | Facilitates structural knowledge sharing across tasks | |
| 27 | [Bai et al. 2025](https://arxiv.org/abs/2505.20972) | Discriminative Models | Graph and Hypergraph Partitioning | Deep-Grouping framework: Unifies graph and hypergraph partitioning, Gini-index-based continuous relaxation annealing | Balances differentiable optimization and discrete convergence | |
| 28 | [Liao et al. 2025](https://arxiv.org/abs/2505.19497) | Discriminative Models | Dynamic Combinatorial Optimization | DyCO-GNN: Temporally continuous structural embedding, GNN parameters shared across temporal snapshots | Enables fast updates on continuous-time graphs without offline training data | |
| 29 | [Min et al. 2025](https://arxiv.org/pdf/2503.20001) | Discriminative Models | Quadratic Assignment Problem | Permutation-invariant method, introduces permutation-symmetric constraints into objective | Enhances model generalization and scalability | |
